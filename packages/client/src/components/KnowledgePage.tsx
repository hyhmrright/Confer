...
function statusBadge(status: string | null) {
  ...
  return (
    <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium border ${colors[s] ?? colors.processing}`}>
      {labels[s] ?? s}
    </span>
  );
}
  ...
  return (
    <div className="rounded-xl border border-dark-border bg-dark-card overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2.5">
        <button className="flex items-center gap-2 flex-1 text-left min-w-0 group" onClick={handleExpand}>
          <ChevronDown
            width={12}
            height={12}
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="px-4 py-3 flex items-center justify-between border-b border-dark-border shrink-0">
        <span className="text-xs font-semibold text-ink-secondary tracking-wider uppercase font-mono">知识库</span>
        <button
