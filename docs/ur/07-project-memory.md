...
> **حالت (2026-09-01)**: یہ دستاویز مطلوبہ صورت بیان کرتی ہے۔ آج جو واقعی موجود ہے وہ **سرور کی جانب کی منصوبہ یادداشت** ہے — MCP کے `read_project_memory` / `write_project_memory` آپ کے اپنے instance کے `project_memory` جدول میں `facts` / `decisions` رکھتے ہیں (صارف × منصوبہ × peer کے اعتبار سے منفرد)، صورت اب بھی Markdown ہے۔ نیچے دی گئی `.claude/` فائل ترتیب، `confer.toml` کا تجزیہ اور حقائق کا خودکار استخراج ابھی نافذ نہیں ہوئے؛ پیش رفت کے لیے [`08-mvp-backlog.md`](./08-mvp-backlog.md) کا v0.2 دیکھیں۔
