...
| `minio` | `quay.io/minio/minio` | S3 ہم آہنگ فائل ذخیرہ |
...
## ڈیسک ٹاپ اور موبائل ایپ

ویب ورژن کو کبھی کسی پتے کی ضرورت نہیں پڑتی: nginx اُسے پیش کرتا ہے اور `/api` اور `/ws` کو
اُسی اوریجن پر پراکسی کرتا ہے۔ پیک شدہ ڈیسک ٹاپ یا Android ایپ مختلف ہے — وہ اپنے وسائل خود
`tauri://localhost` سے پیش کرتی ہے (Windows، Linux اور Android پر یہ `http://tauri.localhost`
لکھا جاتا ہے)، جہاں اضافی `/api/v1` خود ایپ بنڈل پر جا پہنچتا ہے۔ اِس لیے اُسے بتانا پڑتا ہے کہ وہ
کس انسٹینس سے تعلق رکھتی ہے، اور یہ جواب صرف تعیناتی کرنے والے کو معلوم ہوتا ہے۔

پہلی بار چلانے پر لاگ اِن اسکرین پر ایک اضافی خانہ — **انسٹینس کا پتہ** — نظر آتا ہے۔ اوپر والے
جدول کی طرح ہی بھریں:

| آپ کی تعیناتی | کیا لکھنا ہے |
|---|---|
| شائع شدہ امیجز یا کلون سے بلڈ (A/B) | `http://localhost` |
| مقامی ڈیویلپمنٹ (C) | `http://localhost:3000` |
| دور دراز انسٹینس | `confer.example.com` |

بغیر اسکیم کے پتے کو `https://` سمجھا جاتا ہے — سوائے `localhost` اور `127.0.0.1` کے، جنہیں
`http://` پڑھا جاتا ہے، کیونکہ اپنی ہی مشین پر کوئی سرٹیفکیٹ نہیں لگاتا۔ پتہ صرف اُسی ڈیوائس پر محفوظ رہتا ہے، اور دوسرے انسٹینس پر جانے سے لاگ اِن سیشن بھی
ساتھ ہی مٹ جاتا ہے — ٹوکن اُسی gateway کا ہوتا ہے جس نے جاری کیا، اور کہیں اور لے جانے پر 401
کے سوا کچھ نہیں ملے گا۔

gateway کی طرف `/api/v1/*` پر بالکل دو اوریجن کی اجازت ہے: `tauri://localhost` اور
`http://tauri.localhost`۔ اِنہیں صرف صارف کی اپنی مشین پر چلنے والی Tauri ایپ ہی لے سکتی ہے —
کوئی ویب صفحہ اِن کا دعویٰ نہیں کر سکتا — اور یہ API کوکی نہیں بھیجتا (bearer ٹوکن ہیڈر میں جاتا
ہے)۔ چنانچہ یہاں جو کھلتا ہے وہ اُس کوڈ کے لیے پڑھنے کی اجازت ہے جس کے پاس پہلے ہی ٹوکن ہے،
کوئی ماحولیاتی اختیار نہیں۔
