...
> **পরিধি:** এই নির্দেশিকা **এক ইনস্ট্যান্সের, স্ব-হোস্ট করা** ব্যবস্থাকে ধরে, TLS সহ বা ছাড়া (নিচে [HTTPS-এ পরিবেশন](#https-এ-পরিবেশন) দেখুন)। প্রকাশ্য বহু-ভাড়াটে হোস্টিং আর ফেডারেশনের কঠোরীকরণ v0.1-এর পরিধির বাইরে — স্থাপত্যের দিক জানতে `docs/02-architecture.md` দেখুন।
...
| `minio` | `quay.io/minio/minio` | S3-সঙ্গতিপূর্ণ ফাইল সংরক্ষণ |
...
ওয়েব সংস্করণের কোনো ঠিকানা লাগে না: nginx সেটি পরিবেশন করে এবং একই অরিজিনে `/api` ও `/ws`
প্রক্সি করে। প্যাকেজ করা ডেস্কটপ বা Android অ্যাপ আলাদা — সেটি নিজের সম্পদ পরিবেশন করে
`tauri://localhost` থেকে (Windows, Linux ও Android-এ যা লেখা হয় `http://tauri.localhost`),
যেখানে আপেক্ষিক `/api/v1` গিয়ে ঠেকে অ্যাপ বান্ডেলেই। তাই তাকে বলে দিতে হয় সে কোন
ইনস্ট্যান্সের, আর এ উত্তর কেবল যিনি স্থাপন করেছেন তিনিই জানেন।

প্রথমবার চালু করলে লগইন পর্দায় বাড়তি একটি ঘর দেখা যায় — **ইনস্ট্যান্সের ঠিকানা**। উপরের
তালিকার মতো করেই পূরণ করুন:

| আপনার স্থাপন পদ্ধতি | কী লিখবেন |
|---|---|
| প্রকাশিত ইমেজ বা ক্লোন থেকে বিল্ড (A/B) | `http://localhost` |
| স্থানীয় ডেভেলপমেন্ট (C) | `http://localhost:3000` |
| দূরবর্তী ইনস্ট্যান্স | `confer.example.com` |

স্কিম ছাড়া ঠিকানা `https://` ধরে নেওয়া হয় — কেবল `localhost` আর `127.0.0.1` বাদে, যেগুলো
`http://` হিসেবে পড়া হয়, কারণ নিজের মেশিনে কেউ সার্টিফিকেট বসায় না।
ঠিকানাটি কেবল ওই ডিভাইসেই থাকে, আর অন্য ইনস্ট্যান্সে বদলালে লগইন করা সেশনও সঙ্গে মুছে যায় —
টোকেন যে gateway সেটি ইস্যু করেছে তারই, অন্যত্র নিয়ে গেলে ৪০১ ছাড়া কিছু মিলবে না।

gateway-এর দিকে `/api/v1/*`-এ ঠিক দুটি অরিজিন অনুমোদিত: `tauri://localhost` এবং
`http://tauri.localhost`। এ দুটি দখল করতে পারে কেবল ব্যবহারকারীর নিজের মেশিনে চলা Tauri অ্যাপ —
কোনো ওয়েব পাতা এগুলোর দাবি করতে পারে না — আর এই API কুকি বহন করে না (bearer টোকেন হেডারে
যায়)। তাই এখানে যা খোলা হচ্ছে তা হলো ইতিমধ্যেই টোকেন-ধারী কোডের জন্য পড়ার সুযোগ, কোনো
পরিবেশগত কর্তৃত্ব নয়।
...
|---------|--------------------|
| উন্নয়নের পর `postgres` ঘুরে ঘুরে পুনরায় চালু হয় | তার ভলিউম PostgreSQL 16 লিখেছিল। দেখুন [২০২৬-০৮-২৯-এর আগে বানানো ইনস্ট্যান্সের উন্নয়ন](#২০২৬-০৮-২৯-এর-আগে-বানানো-ইনস্ট্যান্সের-উন্নয়ন)। |
