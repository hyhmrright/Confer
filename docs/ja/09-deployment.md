...
| `minio` | `quay.io/minio/minio` | S3 互換のファイルストレージ |
...
Web 版はアドレスを知る必要がありません。nginx が配信し、`/api` と `/ws` を同一オリジンで
プロキシするからです。バンドルされたデスクトップ版と Android 版は違います。自分自身の
アセットを `tauri://localhost`（Windows・Linux・Android では `http://tauri.localhost`）から
配信するため、相対パスの `/api/v1` はアプリバンドル自身に解決されてしまいます。どの
インスタンスに属するのかを教えてもらう必要があり、その答えを知っているのは配備した本人
だけです。

初回起動時、ログイン画面に「インスタンスのアドレス」欄が現れます。上の表と同じ要領で
入力してください：

| 配備方法 | 入力する値 |
|---|---|
| 公開イメージまたはクローンからのビルド（A/B） | `http://localhost` |
| ローカル開発（C） | `http://localhost:3000` |
| リモートのインスタンス | `confer.example.com` |

スキームを省いた場合は `https://` として扱われます。ただし `localhost` と `127.0.0.1` は
`http://` として読まれます — 手元のマシンに証明書を置く人はいないからです。アドレスはその端末にのみ保存され、別のインスタンスに切り替えると
ログイン状態も一緒に消えます。トークンはそれを発行した gateway のものであり、持ち越しても
401 にしかなりません。

gateway 側は `/api/v1/*` で `tauri://localhost` と `http://tauri.localhost` の 2 つの
オリジンだけを許可します。この 2 つを名乗れるのは利用者自身のマシン上の Tauri アプリだけで、
Web ページには不可能です。さらにこの API は Cookie を使わない（ベアラートークンはヘッダーで
送る）ため、ここで開かれるのは「すでにトークンを持つコードがレスポンスを読める」ことであり、
環境的な権限ではありません。
