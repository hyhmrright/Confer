...
> **対象範囲:** 本書が扱うのは **単一インスタンスのセルフホスト**構成で、TLS の有無は問わない（後述の「HTTPS で配信する」を参照）。パブリックなマルチテナント運用とフェデレーションの堅牢化は v0.1 の範囲外である — アーキテクチャ上の方向性は `docs/02-architecture.md` を参照。
...
| 症状 | 考えられる原因 / 対処 |
|---------|--------------------|
| アップグレード後に `postgres` が再起動を繰り返す | そのボリュームは PostgreSQL 16 が書いたものである。「2026-08-29 より前に作られたインスタンスのアップグレード」を参照。 |
