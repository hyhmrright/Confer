...
- リプレイ不可能: リクエストは 5 分間のウィンドウに制限され、検証済みの署名はリプレイノンスキャッシュに記録されるため、ウィンドウ内に同一リクエストを再送しても拒否されます。署名検証だけで送信者の身元を確認できます
...
### スレッドの束縛（`thread_id` のスコープ）

受信メッセージの `thread_id` は peer からの**要求**であり、権威ある指示ではありません。gateway は次の 2 条件が同時に成り立つ場合にのみ再利用し、それ以外は新しい会話を作成します。

1. その peer がすでにその会話の参加者であること。
2. その会話が**宛先 Agent の主人に属している**こと（`conversations.created_by`）。

2 番目の条件は省略できません。`peer_agents` は DID 単位でグローバルに一意なので、1 つの peer が複数の主人と同時に接続できます。1 番目の条件だけでは、A と B の両方に接続した peer が B の Agent 宛にメッセージを送りながら A の `thread_id` を添えることで、A の会話へメッセージを流し込めてしまいます。その場合、B の Agent は A の履歴を文脈として応答し、返信は A のスレッドに書き込まれて A へ配信され、さらに A の会話内容が B の長期記憶へ蓄積されます。

会話を作成する際は、**主人と peer の両方**を `conversation_participants` に書き込みます。主人の参加者行は会話一覧および会話単位の読み取りゲートが参照する根拠であり、これが無いと主人は自分の Agent が応答しているスレッドを見られません。
...
- peer-domain ごとに 1 分あたりのレート制限（プロセス内カウンター。ゲートウェイは単一インスタンス）
