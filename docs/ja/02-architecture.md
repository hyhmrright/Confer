...
> **本書は目標アーキテクチャであり、現在の実装ではありません。** 現行の実装は
> **単一プロセス・単一インスタンス**の gateway です。WebSocket の接続テーブル、
> A2A リプレイ防止用の nonce、レート制限カウンタはすべて gateway プロセスの
> メモリ上にあります（`ws/handler.ts`、`lib/nonce-cache.ts`、
> `middleware/rate-limit.ts`）。
>
> **したがって gateway は 2 つ目のレプリカを動かせません。** レプリカを増やすと
> A2A のリプレイ防止が黙って壊れます — リプレイされたリクエストが別のレプリカに
> 届くと、そこでは nonce テーブルが空なのでそのまま通ってしまいます。さらに WS
> 配信は別レプリカにつながっているユーザーに届かず、レート制限のしきい値は
> レプリカ数だけ倍増します。
>
> 以下に出てくる NATS / Redis は水平スケール時の想定であり、**現時点では配備も
> 結線もされていません**（2026-08-07 に `docker-compose*.yml` と `env.ts` から
> 削除済み。それ以前も空回りするコンテナと、一度も読まれない環境変数でした）。
> 本当に水平スケールするなら、まず上記 3 つのプロセス内状態を共有ストアへ移すこと。
> セキュリティに直結する nonce が最優先です。
...
| コンポーネント | 用途 | 状態 |
|---|---|---|
| PostgreSQL | ユーザー、Agent、対話、メッセージ、権限、peer 関係（メインストレージ） | ✅ 使用中 |
| Qdrant | Agent の長期記憶 RAG、ユーザー資料庫のインデックス | ✅ 使用中 |
| S3-compatible (MinIO) | 知識ベースのファイル保存 | ✅ 使用中 |
| Redis | session、presence、レート制限カウンタ、ホットデータキャッシュ | ⬜ 未配備。水平スケール時にのみ必要 |
| NATS Streams | メッセージのファンアウト（user.{uid}.events）+ Agent runtime のタスクキュー | ⬜ 未配備。水平スケール時にのみ必要 |
