...
| `minio` | `quay.io/minio/minio` | S3-совместимое хранилище файлов |
...
|----------|--------------------------|-------|
| `JWT_SECRET` | `change-me-in-production` | **Смените.** Подписывает токены пользовательских сессий. С этим значением-заглушкой или с любым значением короче 32 символов шлюз не запустится. Сгенерировать: `openssl rand -hex 32`. |
...
> С 2026-09-13 шлюз отказывается запускаться, если `JWT_SECRET` всё ещё равен `change-me-in-production` или короче 32 символов. Перед обновлением старого экземпляра проверьте `.env` и, если значение не подходит, замените его сгенерированным через `openssl rand -hex 32`. После замены все открытые сессии завершатся, и войти придётся заново. Экземпляры, установленные через `npx confer-cli` или скриптом для Oracle, с самого начала получили случайное значение, и их это не касается.
...
Веб-версии адрес не нужен: её отдаёт nginx, он же проксирует `/api` и `/ws` на том же
источнике. Собранное настольное или Android-приложение устроено иначе — оно отдаёт
собственные ресурсы с `tauri://localhost` (на Windows, Linux и Android это пишется как
`http://tauri.localhost`), и относительный `/api/v1` там указывает на сам пакет приложения.
Ему нужно сообщить, какому экземпляру оно принадлежит, а это знает только тот, кто его
развернул.

При первом запуске на экране входа появляется дополнительное поле **«Адрес экземпляра»**.
Заполните его так же, как в таблице выше:

| Ваш способ развёртывания | Что вводить |
|---|---|
| Опубликованные образы или сборка из клона (A/B) | `http://localhost` |
| Локальная разработка (C) | `http://localhost:3000` |
| Удалённый экземпляр | `confer.example.com` |

Адрес без схемы считается `https://` — кроме `localhost` и `127.0.0.1`, которые читаются как
`http://`: сертификат на машине, за которой сидишь, никто не ставит. Адрес хранится только на этом устройстве, а переключение на другой экземпляр
заодно стирает вход — токен принадлежит выдавшему его шлюзу, и перенос его в другое место
может дать только 401.

Со стороны шлюза на `/api/v1/*` разрешены ровно два источника: `tauri://localhost` и
`http://tauri.localhost`. Занять их может только приложение Tauri на машине самого
пользователя — веб-страница на них претендовать не может, — а это API не использует cookie
(bearer-токен отправляется заголовком). Так что здесь открывается доступ на чтение для кода,
который уже располагает токеном, а не какие-либо окружающие полномочия.
