...
> **Область:** это руководство описывает **самостоятельный хостинг одного экземпляра**, с TLS или без него (см. «Отдача по HTTPS» ниже). Публичный многоарендный хостинг и укрепление федерации в область v0.1 не входят — направление развития архитектуры см. в `docs/02-architecture.md`.
...
|---------|--------------------|
| `postgres` после обновления перезапускается по кругу | Его том записан PostgreSQL 16. См. «Обновление экземпляра, созданного до 2026-08-29». |
