...
デスクトップ／モバイルのビルドは各リリースで配布しており、初回起動時にインスタンスの
アドレスを尋ねます。検証は依然として Web クライアントほど行き届いていません。
残りのスコープは [`docs/08-mvp-backlog.md`](../08-mvp-backlog.md) を参照。
